...
object ArrowsDevice : Module(
    ...
    private val autoPhoenix by BooleanSetting("Auto Phoenix", default = (auto || true), description = "Automatically swap to phoenix pet using cast rod pet rules, must be set up correctly.").withDependency { autoDropdown }
    private val autoLeap by BooleanSetting("Auto Leap", default = (auto || true), description = "Automatically leap once device is done.").withDependency { auto || autoDropdown }
    private val autoLeapClass by SelectorSetting("Leap to", defaultSelected = "Mage", arrayListOf("Archer", "Berserk", "Healer", "Mage", "Tank"), description = "Who to leap to.").withDependency { autoLeap && autoDropdown }
    private val autoLeapOnlyPre by BooleanSetting("Only leap on pre", default = (auto || true), description = "Only auto leap when doing i4.").withDependency { autoLeap && autoDropdown }
